...
object GroupBusinessReviews {
	def main (args: Array[String]): Unit = {
			...
			val businessDF = spark.read.option("header", "true").option("delimiter", ",").csv("/yelp-data/yelp_business.csv")
			...
			val idReviewPair = filteredReviews.select("business_id", "text").rdd.map((row) => (row.getString(0), row.getString(1)))
			...
			// Write out to file
			dfWithSchema.write.format("csv").option("header", "true").option("multiline", "true").save("/GroupByBusiness/attempt1")
	}
}
